...
export default function QuestionChoice(
    ...
    const getButtonStyles = () => {
        ...
        if (clickedIndex === null) {
            return `${baseStyles} bg-white border-gray-200 hover:border-blue-400 hover:shadow-lg hover:scale-[1.02] hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 cursor-pointer group`
        }
        
        // Show correct answer in green with success animation
        if (i+1 === correct) {
            return `${baseStyles} bg-gradient-to-r from-green-50 to-emerald-50 border-green-400 text-green-800 cursor-not-allowed animate-success-pulse`
        }
        
        // Show clicked wrong answer in red with shake animation
        if (i === clickedIndex) {
            return `${baseStyles} bg-gradient-to-r from-red-50 to-pink-50 border-red-400 text-red-800 cursor-not-allowed animate-error-shake`
        }
    ...
    const getIndicator = () => {
        ...
        if (i+1 === correct) {
            return (
                <div className="w-10 h-10 rounded-full bg-gradient-to-r from-green-500 to-emerald-500 text-white flex items-center justify-center animate-bounce-once shadow-lg">
        ...
        if (i === clickedIndex) {
            return (
                <div className="w-10 h-10 rounded-full bg-gradient-to-r from-red-500 to-pink-500 text-white flex items-center justify-center animate-pulse shadow-lg">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    return (
        <Fragment key={i}>
            ...
            >
                ...
                {clickedIndex === null && (
                    <div className="absolute inset-0 bg-gradient-to-r from-blue-400 to-purple-400 opacity-0 group-hover:opacity-10 transition-opacity duration-300 rounded-2xl"></div>
                )}
            </button>
        </Fragment>
    )
